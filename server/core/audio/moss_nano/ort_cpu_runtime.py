...
def _normalize_execution_provider(raw_execution_provider: str | None) -> str:
    normalized = str(raw_execution_provider or EXECUTION_PROVIDER_AUTO).strip().lower()
    if normalized in {EXECUTION_PROVIDER_AUTO, "auto", "default"}:
        return EXECUTION_PROVIDER_AUTO
...
def _resolve_ort_providers(execution_provider: str) -> list[Any]:
    ...
    # 优先 GPU / CUDA 调度
    if normalized in {EXECUTION_PROVIDER_CUDA, EXECUTION_PROVIDER_AUTO}:
        ...
        if normalized == EXECUTION_PROVIDER_CUDA:
...
class OrtCpuRuntime:
    ...
    def _session(self, path_value: Path) -> ort.InferenceSession:
        ...
        if not hasattr(self, "actual_providers"):
            self.actual_providers = actual_providers
        if self.execution_provider == EXECUTION_PROVIDER_CUDA and "CUDAExecutionProvider" not in actual_providers:
            raise RuntimeError(
                "CUDAExecutionProvider was requested, but ONNX Runtime created a session without CUDA support "
                f"for {path_value}. Actual providers: {actual_providers}"
            )
        return session
