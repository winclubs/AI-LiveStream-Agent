...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "data" / "voices"
...
try:
    ...
    from .ort_cpu_runtime import (
        ...
        EXECUTION_PROVIDER_AUTO,
        SAMPLE_MODE_FIXED,
        SAMPLE_MODE_FULL,
        SAMPLE_MODE_GREEDY,
    )
except (ImportError, ValueError):
    from server.core.audio.moss_nano.text_normalization_pipeline import WeTextProcessingManager, prepare_tts_request_texts
    from server.core.audio.moss_nano.ort_cpu_runtime import (
        OrtCpuRuntime,
        _normalize_sample_mode,
        _resolve_stream_decode_frame_budget,
        EXECUTION_PROVIDER_CPU,
...
DEFAULT_BROWSER_ONNX_MODEL_DIR = PROJECT_ROOT / "data" / "models" / "moss_tts_nano"
...
DEFAULT_BROWSER_ONNX_TTS_REPO_URL = f"https://hf-mirror.com/{DEFAULT_BROWSER_ONNX_TTS_REPO_ID}"
DEFAULT_BROWSER_ONNX_CODEC_REPO_URL = f"https://hf-mirror.com/{DEFAULT_BROWSER_ONNX_CODEC_REPO_ID}"
...
SENTENCE_END_PUNCTUATION: set[str] = set(".!?。！？；;")
CLAUSE_SPLIT_PUNCTUATION: set[str] = set(",，、；;：:")
CLOSING_PUNCTUATION: set[str] = set("\"'”’)]}）】》」』")
...
def _snapshot_download_repo(
    *,
    ...
    # 兼容新旧版本 huggingface_hub (新版已废弃并移除 local_dir_use_symlinks 参数)
...
def _split_text_by_punctuation(text: str, punctuation: Collection[str]) -> list[str]:
    sentences: list[str] = []
...
class OnnxTtsRuntime(OrtCpuRuntime):
    def __init__(
        ...
        *,
        ...
        execution_provider: str = EXECUTION_PROVIDER_AUTO,
    ...
    def _load_reference_audio(self, reference_audio_path: str | Path) -> np.ndarray:
        ...
        p = Path(reference_audio_path).expanduser().resolve()
        ...
        # 方案 A (高保真人声前处理管道):
        # 1. silenceremove: 自动切除开头 >0.08s 冗余静音与底噪
        # 2. loudnorm: 响度标准化至 -16LUFS (峰值 -1.5dBFS)，让 16 层 RVQ 码本充分激活
        # 3. -t 8.0: 黄金时长限制（最多截取饱满的 8 秒，约 100 帧），防止自注意力散焦与生成卡顿
        # 4. 专业多相 Sinc 滤波重采样为 48000Hz 立体声，彻底消除 FFT 吉布斯高频振铃与金属电音
        try:
            cmd = [
                "ffmpeg", "-y", "-hide_banner", "-loglevel", "error",
                "-i", str(p),
                "-af", "silenceremove=start_periods=1:start_duration=0.08:start_threshold=-40dB,loudnorm=I=-16:TP=-1.5:LRA=11",
                "-t", "8.0",
                "-f", "f32le",
                "-acodec", "pcm_f32le",
            ...
            if len(raw_bytes) >= target_channels * 4 * int(target_sample_rate * 0.5):
                samples = np.frombuffer(raw_bytes, dtype=np.float32)
                waveform = samples.reshape(-1, target_channels).T
        except Exception as exc:
            logging.warning(f"FFmpeg 高保真音频前处理异常 ({exc})，切换至备用重采样逻辑")
            waveform = None

        # 方案 B (安全回退): 若 FFmpeg 处理失败，通过 soundfile 读取并截断至黄金时长
        if waveform is None:
            try:
                ...
                if sample_rate != target_sample_rate:
                    num_target_samples = int(round(waveform.shape[1] * target_sample_rate / sample_rate))
                    x_old = np.linspace(0, 1, waveform.shape[1], endpoint=False)
                    x_new = np.linspace(0, 1, num_target_samples, endpoint=False)
                    resampled_channels = []
                    for c in range(waveform.shape[0]):
                        resampled_channels.append(np.interp(x_new, x_old, waveform[c]))
                    waveform = np.vstack(resampled_channels).astype(np.float32)
            except Exception as e2:
                logging.error(f"备用读取音频依然失败: {e2}")
                raise RuntimeError(f"无法读取参考音频文件: {p}")
    ...
    def synthesize_single_chunk(
        ...
        *,
        ...
        prompt_audio_codes: list[list[int]],
        streaming: bool,
        on_audio_chunk: Callable[[Any], None] | None = None,
    ) -> dict[str, Any]:
        ...
        def decode_pending_frames(force: bool) -> None:
            ...
            emitted_chunk = _merge_audio_channels([audio[0, channel_index, :audio_length] for channel_index in range(audio.shape[1])])
            emitted_chunks.append(emitted_chunk)
            if on_audio_chunk is not None:
                try:
                    on_audio_chunk(emitted_chunk)
                except Exception:
                    pass
