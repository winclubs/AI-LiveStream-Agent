...
### 1. 形象素材拍摄黄金法则 (真人写实核心)
- **设备与规格**：建议使用手机或微单以 **4K 或 1080P 60FPS** 录制一段 **1~2 分钟**的清晰原片短视频；
- **景别与打光**：主播胸部以上入镜（半身人像），正面光线均匀柔和（建议使用环形补光灯或双侧柔光箱），避免背光、强阴影或严重“阴阳脸”；
- **姿态与面部**：
  - 前 5 秒保持闭口微微笑、自然平视镜头，呈现自然待机微呼吸；
  - 随后自然开口朗读任意一段流畅文本（无需刻意用力夸张）；
- **音频环境**：在安静隔音环境下录制清晰的 16kHz 单声道人声，杜绝环境杂音与回音。此音频将直接作为伴音分轨基准及 10 秒音色复刻源；
- **背景选择**：支持纯色实景背景或绿幕背景。若使用绿幕，幕布需平整无褶皱且打光均匀，方便在 OBS 或直播伴侣中执行一键色度键抠像。
